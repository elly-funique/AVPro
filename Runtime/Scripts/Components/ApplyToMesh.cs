...
namespace RenderHeads.Media.AVProVideo
{
	...
	public class ApplyToMesh : ApplyToBase
	{
        ...
        {
            get { return forceDefault; }
            set { forceDefault = value; }
        }
        ...
        public Texture LastTextureApplied
        {
            get { return _lastTextureApplied; }
        }
		...
		public override void Apply()
		{
			...
			{
                Texture resamplerTex = GetResampleTexture();
                ...
                if (texture != null)
				{
					...
					if (_isDirty)
					{
						...
						{
                            Texture resamplerTexPlane = GetResampleTexture(plane);
                            texture = GetTexture(plane);
                            if (texture != null)
							{
								flip = _media.TextureProducer.RequiresVerticalFlip();

                                ApplyMapping(texture, flip, plane, _materialIndex);
							}
						}
					}
					applied = true;
			...
			if (!applied)
			{
				...
				if (_isDirty)
				{
					ApplyMapping(_defaultTexture, flip, 0, _materialIndex);
				}
			}
		}
					...
					for (int i = 0; i < meshMaterials.Length; i++)
					{
						if (_materialIndex < 0 || i == _materialIndex)
						{
							...
							if (mat != null)
							{
								if (plane == 0)
								{
									VideoRender.SetupMaterialForMedia(mat, _media, _propTexture.Id, texture, forceDefault ? true : texture == _defaultTexture);
        ...
        public virtual Texture GetResampleTexture()
        {
            return GetResampleTexture(0);
        }
        public virtual Texture GetResampleTexture(int index)
        {
            return _media.FrameResampler == null || _media.FrameResampler.OutputTexture == null ? null : _media.FrameResampler.OutputTexture[index];
        }
        ...
        {
            return _media.UseResampler ? GetResampleTexture(index) : _media.TextureProducer.GetTexture(index);
        }
    }
}